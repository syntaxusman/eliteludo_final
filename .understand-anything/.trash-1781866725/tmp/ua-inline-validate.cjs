...
try {
  ...
  if (!Array.isArray(graph.layers)) { graph.layers = []; issues.push('graph.layers is not an array'); }
  if (!Array.isArray(graph.tour)) { graph.tour = []; issues.push('graph.tour is not an array'); }
